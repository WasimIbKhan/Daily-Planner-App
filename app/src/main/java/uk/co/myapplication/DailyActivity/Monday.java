 ...
 */
public class Monday extends Fragment implements FloatingActionButton.OnClickListener, ContentAdapter.OnContentListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View root = (ViewGroup) inflater.inflate(R.layout.fragment_monday, container, false);
        database = new ContentDatabase(getActivity());
        setComparator();
        contents= database.getContents("MONDAY");
        addEvents();
        Collections.sort(contents, byTime);
        FloatingActionButton addPermanentContentBtn = root.findViewById(R.id.addPermanentContentMonday);
        FloatingActionButton addTemporaryContentBtn=root.findViewById(R.id.addTemporaryContentMonday);
        ...
        switchButton=root.findViewById(R.id.toggleMonday);
        recyclerView=root.findViewById(R.id.MondayRecyclerView);
    ...
    private void addEvents() {
        ...
        for(Content content:eventContents){
            try {
                if(convertStringToDate(dates.getMonday()).compareTo(convertStringToDate(content.getDate()))==0){
                    if(!isPresent){
                        AddScheduleTask addScheduleTask= new AddScheduleTask(getActivity(),content,"MONDAY");
                        addScheduleTask.addNotification();
                        content.setNotificationID(addScheduleTask.getNotificationID());
                        database.addContent(content, "MONDAY");
                    }
                }
    ...
    private void deleteContent(Content content) {
        if(dates.getMonday().equals(dates.getToday())){
        ...
        database.deleteContent(id,"MONDAY");
    }
    ...
    private void moveDialog(final Dialog epicDialog, final int position) {
        ...
        completePopupBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                setRecyclerView(database.getContents("MONDAY"));
                epicDialog.dismiss();
            }
        });
        closePopupBtn=(ImageButton) epicDialog.findViewById(R.id.CloseBtn);
