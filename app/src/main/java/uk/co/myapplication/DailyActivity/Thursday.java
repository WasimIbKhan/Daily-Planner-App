 ...
 */
public class Thursday extends Fragment implements FloatingActionButton.OnClickListener, ContentAdapter.OnContentListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View root = (ViewGroup) inflater.inflate(R.layout.fragment_thursday, container, false);
        database = new ContentDatabase(getActivity());
        setComparator();
        contents= database.getContents("THURSDAY");
        addEvents();
        Collections.sort(contents, byTime);
        FloatingActionButton addPermanentContentBtn = root.findViewById(R.id.addPermanentContentThursday);
        FloatingActionButton addTemporaryContentBtn=root.findViewById(R.id.addTemporaryContentThursday);
        ...
        switchButton=root.findViewById(R.id.toggleThursday);
        recyclerView=root.findViewById(R.id.ThursdayRecyclerView);
    ...
    private void addEvents() {
        ...
        for(Content content:eventContents){
            try {
                if(convertStringToDate(dates.getThursday()).compareTo(convertStringToDate(content.getDate()))==0){
                    if(!isPresent){
                        AddScheduleTask addScheduleTask= new AddScheduleTask(getActivity(),content,"THURSDAY");
                        addScheduleTask.addNotification();
                        content.setNotificationID(addScheduleTask.getNotificationID());
                        database.addContent(content, "THURSDAY");
                    }
                }
    ...
    private void deleteContent(Content content) {
        if(dates.getThursday().equals(dates.getToday())){
        ...
        database.deleteContent(id,"THURSDAY");
    }
    ...
    private void moveDialog(final Dialog epicDialog, final int position) {
        ...
        completePopupBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                setRecyclerView(database.getContents("THURSDAY"));
                epicDialog.dismiss();
            }
        });
        closePopupBtn=(ImageButton) epicDialog.findViewById(R.id.CloseBtn);
