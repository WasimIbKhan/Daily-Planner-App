 ...
 */
public class Tuesday extends Fragment implements FloatingActionButton.OnClickListener, ContentAdapter.OnContentListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View root = (ViewGroup) inflater.inflate(R.layout.fragment_tuesday, container, false);
        database = new ContentDatabase(getActivity());
        setComparator();
        contents= database.getContents("TUESDAY");
        addEvents();
        Collections.sort(contents, byTime);
        FloatingActionButton addPermanentContentBtn = root.findViewById(R.id.addPermanentContentTuesday);
        FloatingActionButton addTemporaryContentBtn=root.findViewById(R.id.addTemporaryContentTuesday);
        ...
        switchButton=root.findViewById(R.id.toggleTuesday);
        recyclerView=root.findViewById(R.id.TuesdayRecyclerView);
    ...
    private void addEvents() {
        ...
        for(Content content:eventContents){
            try {
                if(convertStringToDate(dates.getTuesday()).compareTo(convertStringToDate(content.getDate()))==0){
                    if(!isPresent){
                        AddScheduleTask addScheduleTask= new AddScheduleTask(getActivity(),content,"TUESDAY");
                        addScheduleTask.addNotification();
                        content.setNotificationID(addScheduleTask.getNotificationID());
                        database.addContent(content, "TUESDAY");
                    }
                }
    ...
    private void deleteContent(Content content) {
        if(dates.getTuesday().equals(dates.getToday())){
        ...
        database.deleteContent(id,"TUESDAY");
    }
    ...
    private void moveDialog(final Dialog epicDialog, final int position) {
        ...
        completePopupBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                setRecyclerView(database.getContents("TUESDAY"));
                epicDialog.dismiss();
            }
        });
        closePopupBtn=(ImageButton) epicDialog.findViewById(R.id.CloseBtn);
