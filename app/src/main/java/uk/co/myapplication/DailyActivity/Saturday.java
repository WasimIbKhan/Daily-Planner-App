 ...
 */
public class Saturday extends Fragment implements FloatingActionButton.OnClickListener, ContentAdapter.OnContentListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View root = (ViewGroup) inflater.inflate(R.layout.fragment_saturday, container, false);
        database = new ContentDatabase(getActivity());
        setComparator();
        contents= database.getContents("SATURDAY");
        addEvents();
        Collections.sort(contents, byTime);
        FloatingActionButton addPermanentContentBtn = root.findViewById(R.id.addPermanentContentSaturday);
        FloatingActionButton addTemporaryContentBtn=root.findViewById(R.id.addTemporaryContentSaturday);
        ...
        switchButton=root.findViewById(R.id.toggleSaturday);
        recyclerView=root.findViewById(R.id.SaturdayRecyclerView);
        ...
        switchButton.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener()
        {
            ...
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                if(checkedId ==R.id.permanentSwitch){
                    ...
                    createTemporary();
                    isPermanent=false;
                    permanentSwitch.setTextColor(Color.parseColor("#03DAC5"));
    ...
    private void addEvents() {
        ...
        for(Content content:eventContents){
            try {
                if(convertStringToDate(dates.getSaturday()).compareTo(convertStringToDate(content.getDate()))==0){
                    if(!isPresent){
                        AddScheduleTask addScheduleTask= new AddScheduleTask(getActivity(),content,"SATURDAY");
                        addScheduleTask.addNotification();
                        content.setNotificationID(addScheduleTask.getNotificationID());
                        database.addContent(content, "SATURDAY");
                    }
                }
    ...
    private void setComparator() {
        byTime = new Comparator<Content>() {
            public int compare(Content c1, Content c2) {
                try {
                    return Long.valueOf(convertStringToTime(c1.getTimeStart()).getTime()).compareTo(convertStringToTime(c2.getTimeStart()  +":00").getTime());
                } catch (ParseException e) {
                    e.printStackTrace();
    ...
    private void deleteContent(Content content) {
        if(dates.getSaturday().equals(dates.getToday())){
        ...
        database.deleteContent(id,"SATURDAY");
    }
    ...
    private void moveDialog(final Dialog epicDialog, final int position) {
        ...
        completePopupBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                setRecyclerView(database.getContents("SATURDAY"));
                epicDialog.dismiss();
            }
        });
        closePopupBtn=(ImageButton) epicDialog.findViewById(R.id.CloseBtn);
