 ...
 */
public class Sunday extends Fragment implements FloatingActionButton.OnClickListener, ContentAdapter.OnContentListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View root = (ViewGroup) inflater.inflate(R.layout.fragment_sunday, container, false);
        database = new ContentDatabase(getActivity());
        setComparator();
        contents= database.getContents("SUNDAY");
        addEvents();
        Collections.sort(contents, byTime);
        FloatingActionButton addPermanentContentBtn = root.findViewById(R.id.addPermanentContentSunday);
        FloatingActionButton addTemporaryContentBtn=root.findViewById(R.id.addTemporaryContentSunday);
        ...
        switchButton=root.findViewById(R.id.toggleSunday);
        recyclerView=root.findViewById(R.id.SundayRecyclerView);
    ...
    private void addEvents() {
        ...
        for(Content content:eventContents){
            try {
                if(convertStringToDate(dates.getSunday()).compareTo(convertStringToDate(content.getDate()))==0){
                    if(!isPresent){
                        AddScheduleTask addScheduleTask= new AddScheduleTask(getActivity(),content,"SUNDAY");
                        addScheduleTask.addNotification();
                        content.setNotificationID(addScheduleTask.getNotificationID());
                        database.addContent(content, "SUNDAY");
                    }
                }
    ...
    private void deleteContent(Content content) {
        if(dates.getSunday().equals(dates.getToday())){
        ...
        database.deleteContent(id,"SUNDAY");
    }
    ...
    private void moveDialog(final Dialog epicDialog, final int position) {
        ...
        completePopupBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                setRecyclerView(database.getContents("SUNDAY"));
                epicDialog.dismiss();
            }
        });
        closePopupBtn=(ImageButton) epicDialog.findViewById(R.id.CloseBtn);
