 ...
 */
public class Friday extends Fragment implements FloatingActionButton.OnClickListener, ContentAdapter.OnContentListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View root = (ViewGroup) inflater.inflate(R.layout.fragment_friday, container, false);
        database = new ContentDatabase(getActivity());
        setComparator();
        contents= database.getContents("FRIDAY");
        addEvents();
        Collections.sort(contents, byTime);
        FloatingActionButton addPermanentContentBtn = root.findViewById(R.id.addPermanentContentFriday);
        FloatingActionButton addTemporaryContentBtn=root.findViewById(R.id.addTemporaryContentFriday);
        ...
        switchButton=root.findViewById(R.id.toggleFriday);
        recyclerView=root.findViewById(R.id.FridayRecyclerView);
    ...
    private void addEvents() {
        ...
        for(Content content:eventContents){
            try {
                if(convertStringToDate(dates.getFriday()).compareTo(convertStringToDate(content.getDate()))==0){
                    if(!isPresent){
                        content.setPriority(3);
                        AddScheduleTask addScheduleTask= new AddScheduleTask(getActivity(),content,"FRIDAY");
                        addScheduleTask.addNotification();
                        content.setNotificationID(addScheduleTask.getNotificationID());
                        database.addContent(content, "FRIDAY");
                    }
                }
    ...
    private void setComparator() {
        byTime = new Comparator<Content>() {
            public int compare(Content c1, Content c2) {
                try {
                    return Long.valueOf(convertStringToTime(c1.getTimeStart()).getTime()).compareTo(convertStringToTime(c2.getTimeStart()).getTime());
                } catch (ParseException e) {
                    e.printStackTrace();
    ...
    private void deleteContent(Content content) {
        if(dates.getFriday().equals(dates.getToday())){
        ...
        database.deleteContent(id,"FRIDAY");
    }
